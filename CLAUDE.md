...
│       └── 008-skia-web-canvaskit-loader.md
...
Note: the app uses Expo SDK 56 / Reanimated 4 / Skia 2.x — newer than the versions named in `context/01_architecture.md` (SDK 53 era). The sprint-02 doc has the reconciliation table. ADR-006 (manual gesture port), ADR-007 (procedural assets), and ADR-008 (Skia web CanvasKit loader) record where the implementation deliberately diverges from the architecture doc.

**Testing surfaces:** the app cannot run in **Expo Go** — Skia is a native module Expo Go does not bundle. Use an EAS development/preview build for devices (the Android `preview` profile installs with no developer account), or the web build for a credential-free browser check. Web is deployed to EAS Hosting at https://fidget.expo.app via `cd FidgetApp && npx expo export -p web && eas deploy` (`--prod` for the stable alias); it needs `canvaskit.wasm` in `public/`, regenerated by `npm run setup:skia-web` (also on `postinstall`). See ADR-008.
